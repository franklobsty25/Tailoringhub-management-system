...
(function () {
    ...
    var subPlaceholder = function (owner, detail, compSpec, placeholders) {
      ...
      return get$c(placeholders, compSpec.name).fold(function () {
        throw new Error('Unknown placeholder components: ' + compSpec.name + '\nKnown: [' + keys(placeholders) + ']\nNamespace: ' + owner.getOr('none') + '\nSpec: ' + JSON.stringify(compSpec, null, 2));
      }, function (newSpec) {
              ...
              getValue: function (form) {
                ...
                return map$1(resPs, function (resPThunk, pName) {
                  return resPThunk().bind(function (v) {
                    ...
                    return toResult(opt, new Error('Cannot find a current components to extract the value from for form part \'' + pName + '\': ' + element(v.element)));
    ...
    var NoContextApi = function (getComp) {
      var getMessage = function (event) {
        return 'The components must be in a context to execute: ' + event + (getComp ? '\n' + element(getComp().element) + ' is not in context.' : '');
      };
    ...
    var generateFrom$1 = function (spec, all) {
      ...
      var validated = asRaw('components.behaviours', objOf(schema), spec.behaviours).fold(function (errInfo) {
    ...
    var external = function (spec) {
      var extSpec = asRawOrDie$1('external.components', objOfOnly([
        required$1('element'),
        option('uid')
      ]), spec);
    ...
    var takeover$1 = function (root) {
      ...
      var getByUid = function (uid) {
        return registry.getById(uid).fold(function () {
          return Result.error(new Error('Could not find components with uid: "' + uid + '" in system.'));
        }, Result.value);
      };
